<?php
/**
 * Culture - Branch of MonoBook which has many usability improvements and
...
/**
 * SkinTemplate class for Culture skin
 * @ingroup Skins
 */


class SkinCulture extends SkinTemplate {

	/* Functions */
	var $skinname = 'culture', $stylename = 'culture',
		$template = 'CultureTemplate', $useHeadElement = true;

	/* shorthands */
	public function tooltipAndAccesskey($xmlID) {
		return Xml::expandAttributes( Linker::tooltipAndAccesskeyAttribs( $xmlID ) );
	}
	public function tooltip($xmlID) {
		return Linker::tooltip( $xmlID );
	}

	public function addToBodyAttributes( $out, &$bodyAttrs ) {
         if($this->loggedin) $bodyAttrs['class'] .= ' loggedin';
        }
	...
	public function initPage( OutputPage $out ) {
		global $wgStylePath, $wgJsMimeType, $wgStyleVersion, $wgScriptPath, $wgCultureExtraStyles;
		
		parent::initPage( $out );
		//hack - why is ther no mTitle?
		if (!$this->mTitle) $this->mTitle = $this->getTitle();
		//hack end
		if(MWNamespace::isContent($this->mTitle->getNamespace()) and $this->mTitle->getText() != "Main Page") {
			$out->addMeta('description','An article about ' . $this->mTitle->getText() . ' from Culture.si, the encyclopedia of Slovenian culture.');
		} else {
			$out->addMeta('description','Culture.si, the encyclopedia of Slovenian culture.');
		}
		$out->addMeta('keywords','Slovenia, culture, culture of Slovenia, Slovene culture, Slovenian culture, artists, venues, producers, heritage, cultural heritage, international cooperation, ministry of culture, arts, art, arts & culture');
		// Append skin-specific styles
		$out->addStyle( 'culture/main-rtl.css', 'screen', '', 'rtl' );
		$out->addStyle( 'culture/main-ltr.css', 'screen', '', 'ltr' );
		...
		$out->addScript(
			'<!--[if lt IE 7]><style type="text/css">body{behavior:url("' .
				$wgStylePath .
				'/culture/csshover.htc")}</style><![endif]-->'
		);
		// Add extra stylesheets
		// THIS IS ONLY USEFUL FOR EXPERIMENTING WITH DIFFERNT STYLE OPTIONS! THIS WILL BE REMOVED IN THE NEAR FUTURE.
		if ( is_array( $wgCultureExtraStyles ) ) {
			foreach ( $wgCultureExtraStyles as $style ) {
				$out->addStyle( 'culture/' . $style, 'screen' );
			}
		}
	...
	function buildNavigationUrls() {
		...
		global $wgDisableLangConversion, $wgCultureUseIconWatch;
		...
		if( $this->iscontent ) {
			...
			$links['namespaces'][$subjectId] = $this->tabAction(
				$subjectPage, 'culture-namespace-' . $subjectId, !$isTalk, '', true
			);
			$links['namespaces'][$subjectId]['context'] = 'subject';
			$links['namespaces'][$talkId] = $this->tabAction(
				$talkPage, 'culture-namespace-talk', $isTalk, '', true
			);
			...
			if ( $this->mTitle->exists() ) {
				$links['views']['view'] = $this->tabAction(
					$isTalk ? $talkPage : $subjectPage,
						'culture-view-view', ( $action == 'view' ), '', true
				);
			}
			...
			if (
				...
				$links['views']['edit'] = array(
					...
					'text' => $this->mTitle->exists()
						? wfMsg( 'culture-view-edit' )
						: wfMsg( 'culture-view-create' ),
					'href' =>
				...
				if ( ( $isTalk && $wgArticle->isCurrent() ) || ( $wgOut->showNewSectionLink() ) ) {
					...
					if ( !$wgOut->forceHideNewSectionLink() ) {
						...
						$links['views']['addsection'] = array(
							...
							'text' => wfMsg( 'culture-action-addsection' ),
			...
			// Checks if the page is known (some kind of viewable content)
				...
				$links['views']['viewsource'] = array(
					...
					'text' => wfMsg( 'culture-view-viewsource' ),
					'href' =>
						$this->mTitle->getLocalUrl( $this->editUrlOptions() )
				);
			}
			...
			if ( $this->mTitle->exists() ) {
				...
				$links['views']['history'] = array(
					...
					'text' => wfMsg( 'culture-view-history' ),
				...
				if( $wgUser->isAllowed( 'delete' ) ) {
					$links['actions']['delete'] = array(
						...
						'text' => wfMsg( 'culture-action-delete' ),
				...
				if ( $this->mTitle->quickUserCan( 'move' ) ) {
					...
					$links['actions']['move'] = array(
						'class' => $this->mTitle->isSpecial( 'Movepage' ) ?
										'selected' : false,
						'text' => wfMsg( 'culture-action-move' ),
					...
					if ( !$this->mTitle->isProtected() ){
						$links['actions']['protect'] = array(
							'class' => ($action == 'protect') ?
											'selected' : false,
							'text' => wfMsg( 'culture-action-protect' ),
							'href' =>
						...
						$links['actions']['unprotect'] = array(
							'class' => ($action == 'unprotect') ?
											'selected' : false,
							'text' => wfMsg( 'culture-action-unprotect' ),
							'href' =>
			 ...
			 * the Culture skin, would break the Ajax code which makes this
		...
		// If it's not content, it's got to be a special page
			...
			$links['namespaces']['special'] = array(
				'class' => 'selected',
				'text' => wfMsg( 'culture-namespace-special' ),
...
/**
 * QuickTemplate class for Culture skin
 * @ingroup Skins
 */
class CultureTemplate extends QuickTemplate {
	...
	public function execute() {
		...
		foreach ( $this->data['personal_urls'] as $key => $item) {
			...
			$this->data['personal_urls'][$key]['key'] =
				Xml::expandAttributes( Linker::tooltipAndAccesskeyAttribs('pt-'.$key) );
		}
...
?>
		...
		<!-- content -->
		<div id="content" <?php $this->html('specialpageattributes'); if ($this->loggedin) echo 'class="LOGGEDIN"'; ?>>
			<a id="top"></a>
			...
			<?php foreach( $validFooterLinks as $category => $links ): ?>
				...
				<ul id="footer-<?php echo $category ?>">
					<?php foreach( $links as $link ): ?>
						...
						<?php endif; ?>
					<?php endforeach; ?>
					<?php if($category=='places') print wfMsgWikiHtml('footer');?>
				</ul>
				<?php endif; ?>
			<?php endforeach; ?>
...
<div class="portal" id="p-tb">
	...
	<div class="body">
		...
		<?php endif; ?>
		<?php wfRunHooks( 'CultureTemplateToolboxEnd', array( &$this ) ); ?>
...
<?php
	...
	private function renderNavigation( $elements ) {
		global $wgContLang, $wgCultureUseSimpleSearch, $wgStylePath;
		...
		foreach ( $elements as $name => $element ) {
			...
			switch ( $element ) {
				case 'NAMESPACES':
?>
<div id="p-namespaces" class="cultureTabs<?php if ( count( $this->data['namespace_urls'] ) == 0 ) echo ' emptyPortlet'; ?>">
...
<?php
				...
				case 'VARIANTS':
?>
<div id="p-variants" class="cultureMenu<?php if ( count( $this->data['variant_urls'] ) == 0 ) echo ' emptyPortlet'; ?>">
...
<?php
				...
				case 'VIEWS':
?>
<div id="p-views" class="cultureTabs<?php if ( count( $this->data['view_urls'] ) == 0 ) echo ' emptyPortlet'; ?>">
...
<?php
				...
				case 'ACTIONS':
?>
<div id="p-cactions" class="cultureMenu<?php if ( count( $this->data['action_urls'] ) == 0 ) echo ' emptyPortlet'; ?>">
...
<div id="p-search">
	...
	<form action="<?php $this->text( 'wgScript' ) ?>" id="searchform">
		...
		<?php if ( $wgCultureUseSimpleSearch ): ?>
		<div id="simpleSearch">
		...
		<input type='hidden' name="ns14" value="1"/>
		<input type='hidden' name="advanced" value="1"/>
		<input type='hidden' name="redirs" value="1"/>

		<input id="searchInput" name="search" type="text" placeholder="<?php $this->msg( 'searchtip' ) ?>" <?php echo $this->skin->tooltipAndAccesskey( 'search' ); ?> <?php if( isset( $this->data['search'] ) ): ?> value="<?php $this->text( 'search' ) ?>"<?php endif; ?> />
