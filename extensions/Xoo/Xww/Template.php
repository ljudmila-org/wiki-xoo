	...
	function frameFunctions($fnName,&$frame,$arg=false)
	{
	  global $wgTitle;
		...
		case 'editcallurl':
			...
			if ($frame->parent)
			  $url = $frame->parent->title->getFullUrl("action=edit&startpos={$frame->startPos}&endpos={$frame->endPos}" );
			  if ($fnName=='editcallurl') return array(noparse =>true, 0=>$url);
				return array
				(
					'isHTML'=>true,
#					'isChildObj'=>true,
					'<span class="editcall"><a href="' . $url.'">'.$editText.'</a></span>'
				);
			return $this->notFound();
