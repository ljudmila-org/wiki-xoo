...
/*
  	...
  	class MyExtension extends XxxExtension
	{
		...
		function var_foo  (&$parser, &$cache, &$magicWordId, &$ret) {};
...
$wgHooks['LanguageGetMagic'][] = array( &$wgXxxInstaller,'hookGetMagicWords');
$wgHooks['MagicWordwgVariableIDs'][] = array( &$wgXxxInstaller,'hookVariableIDs');
$wgHooks['ParserGetVariableValueSwitch'][] = array(&$wgXxxInstaller,'hookVariableValueSwitch');
...
######################################################################
	...
	static function Install($x)
	{
		XxxInstaller::Install($x);
	}
}
...
################################
	...
	{
		global $wgXooExtensions;
		if (isset($this->mExtensions[$className])) 
		{
			warn("Xxx: Extension $className already registered.");
			return false;
		}
		$this->mExtensions[] = $className;
		$wgXooExtensions[$className] = new $className;
		$this->autoRegister($wgXooExtensions[$className]);
	}
	...
	function hookGetMagicWords( &$magicWords, $langCode=0) 
	{
