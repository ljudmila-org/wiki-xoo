		...
		case 'SELECT':
			...
			foreach($this->mFields as $k=>$v)
			{
				...
				if ($check!=$v)
				{
					...
					$fields = array (
						$this->escapeName("{$tableAlias}")."._row_ref AS ".$this->escapeName("{$fieldPrefix}#"),
						$this->escapeName("{$tableAlias}")."._row_ref AS ".$this->escapeName("{$fieldPrefix}#ref"),
						$this->escapeName("{$tableAlias}")."._row_name AS ".$this->escapeName("{$fieldPrefix}#name"),
						$this->escapeName("{$tableAlias}")."._page_title AS ".$this->escapeName("{$fieldPrefix}#page")
					);
					foreach ($fieldNames as $f)
					{
					   if ($fieldDefs['fieldsByName'][$f]['field_type']=='multi') continue;
						$fields[]="(".$this->escapeName($tableAlias)."." . $this->escapeName($f) . ") AS " . $this->escapeName("$fieldPrefix$f");
					}
				}
				else
				{
