...
###################################
		...
		case 'table':
			...
			for ($i = 1; $i<=$args->count; $i++) {
				if ($args->isNamed($i)) {
					...
					if (!$this->removePrefix($argValue,'#')) {
						...
						if (!Xtt::getDbFieldType($fieldType,$this->S('dbtype'))) {
			...
			$res = $dbr->select
			(
				$this->getInternalTableName('fields'),
				'*',
...
##############################################################################
		...
		case 'schema':
			...
			default:
				$dbr = $this->getDbr();
				$fieldTable = $this->getInternalTableName('fields');
				$ret="{\n";
				$sql = "
SELECT * FROM (
	(
		...
		SELECT 	field_reference AS ta, 
			field_reverse AS fi, 
			".($wgDBtype=='mysql' ? " CONCAT('rev_',field_type) AS ty " : "'rev_' || field_type AS ty ").", 
			field_table AS re
		FROM $fieldTable 
...
##############################################################################
		...
		case 'selectcolumn':
			...
			case 'grid':
				...
				catch(Exception $e){ return "<pre>$sql</pre>".$e->error."<br>";}
			...
			case 'selectcolumn':
			   global $wgXooExtensions;
				try
				{	
  				if ($dbr->trxLevel() ) $dbr->query('SAVEPOINT wikidataselect;',__METHOD__,true);
          //$ignore = $dbr->ignoreErrors(true);
					@$res=$dbr->query($sql,__METHOD__,true);
					//$dbr->ignoreErrors($ignore);
					if(!$res) {
					  $returnText.="<pre> Error:".$dbr->lastError()."</pre>"; 
    				if ($dbr->trxLevel() ) $dbr->query('ROLLBACK TO SAVEPOINT wikidataselect;',__METHOD__,true);
					  break;
          }
  				if ($dbr->trxLevel() ) $dbr->query('RELEASE SAVEPOINT wikidataselect;',__METHOD__,true);
	...
	function getDbr()
	{
		...
		if (!$this->mDbr) 
		{
			if ($S['dbhost']!=$wgDBserver or $S['dbname']!=$wgDBname or $S['dbuser']!=$wgDBuser) {
 				$this->mDbr = Database::factory( $S['dbtype'],array('host'=>$S['dbhost'], 'dbname'=>$S['dbname'], 'user'=>$S['dbuser'], 'password'=>$S['dbpass']));
...
#######################	
	...
	{  
		return $this->esc_ident.trim(preg_replace('/'.$this->esc_ident.'/',$this->esc_esc_ident,$s)).$this->esc_ident;
	}
	function escapeDataTableName($s)
	{
		return $this->esc_ident.$this->S('tableprefix').trim(preg_replace('/'.$this->esc_ident.'/',$this->esc_esc_ident,$s)).$this->esc_ident;
	}
	function escapeInternalTableName($s)
	{
		return $this->esc_ident.$this->S('internalprefix').trim(preg_replace('/'.$this->esc_ident.'/',$this->esc_esc_ident,$s)).$this->esc_ident;
	}
	...
	function escapeValue($s)
	{
		...
		return $this->esc_value.trim(preg_replace('/'.$this->esc_value.'/',$this->esc_esc_value,$s)).$this->esc_value;
	}
...
//echo "table EXists: $tableName<br>";
...
//echo '$fieldDefs <pre>'.print_r($fieldDefs,1).'</pre>';
...
*/							}
	...
	function suggestField($tableName,$fieldName)
	{
		...
		if (!$fieldNames)
		{
			...
			$res = $dbr->query ('SELECT DISTINCT field_table FROM ' . $this->getInternalTableName('fields'));
	...
	{	
		...
		case 'CREATE':
			return "\n\t"  
				 ...
				 . Xtt::getDbFieldType($fieldDef['field_type'],$this->S('dbtype'));
		case 'ADD':
			return "\n\tADD COLUMN "  
				 . $this->escapeName($fieldDef['field_name']) . " "
				 . Xtt::getDbFieldType($fieldDef['field_type'],$this->S('dbtype'));
...
//						. ($fieldDef['field_type']=='text' ? '(255)' : '');
				...
				return "\n\tALTER COLUMN "  
						. $this->escapeName($fieldDef['field_name']) . " "
						. "TYPE ".Xtt::getDbFieldType($fieldDef['field_type'],$this->S('dbtype')) . " "
						. "USING ".Xtt::getDbFieldDefault($fieldDef['field_type']);
			}
		}
	}
	...
	{	
		$dbr = $this->getDbr();
		$multiTable = $this->S('tableprefix').$this->getMultiTableName($tableName,$fieldName);
		$escMultiTableName=$this->escapeMultiTableName($tableName,$fieldName);
//echo "multiTableName $op :: $escMultiTableName ::".($dbr->tableExists($multiTable) ? 'TRUE' : 'NONE')."<br>";
		switch ($op)
		{
		case 'ADD':
			if ($dbr->tableExists($multiTable)) return "";
			return "CREATE TABLE " . $escMultiTableName . "\n" 
					. "( _multi_from ".Xtt::getDbFieldType('text',$this->S('dbtype')).", _multi_to ".Xtt::getDbFieldType('text',$this->S('dbtype')).", _multi_page_id ".Xtt::getDbFieldType('integer',$this->S('dbtype')).")"; 
		case 'DROP':
			if (!$dbr->tableExists($multiTable)) return "";
			return "DROP TABLE " . $escMultiTableName;
		}
		return "";
	}
	
	function makeMultiIndex($tableName,$fieldName)
	{	
		$dbr = $this->getDbr();
		$escMultiTableName=$this->escapeMultiTableName($tableName,$fieldName);
		$multiTableName = $this->S('tableprefix').$this->getMultiTableName($tableName,$fieldName);
		$escMultiIndexName=$this->escapeMultiTableName($tableName.$fieldName,$fieldName);
		$multiIndexName = $this->S('tableprefix').$this->getMultiTableName($tableName.$fieldName,$fieldName);
		$ignore = $dbr->ignoreErrors(true);
		if ($dbr->indexExists($multiTableName,$multiIndexName)) return "";
		$dbr->ignoreErrors($ignore);
		return "CREATE INDEX " . $escMultiIndexName .' on '. $escMultiTableName . " (_multi_from,_multi_to,_multi_page_id)";
	}
	...
	function saveTableData(&$parserOutput,$tableTitle,$pageId,$getSqlOnly=false)
	{
		...
		if (!$tableDef) $tableDef = $this->makeTableDef($tableName,array());
...
//		   if ( $this->tableExists($this->escapeDataTableName($tableName)) ) 
			$sql= "CREATE TABLE " . $this->escapeDataTableName($tableName) . "\n(\n";
			$sql.= "\t_page_id\t ".Xtt::getDbFieldType('integer',$this->S('dbtype'))." NOT NULL,\n";
			$sql.= "\t_page_ns\t".Xtt::getDbFieldType('text',$this->S('dbtype'))." NOT NULL,\n";
			$sql.= "\t_page_title\t".Xtt::getDbFieldType('text',$this->S('dbtype'))." NOT NULL, \n";
			$sql.= "\t_row_name\t".Xtt::getDbFieldType('text',$this->S('dbtype'))." NOT NULL,\n";
			$sql.= "\t_row_ref \t".Xtt::getDbFieldType('text',$this->S('dbtype'))." NOT NULL PRIMARY KEY";
			...
			{
        print ("<pre>$sql</pre>");
			   $fname = $this->escapeName($fieldDef['field_name']);
			   $ftype = ($fieldDef['field_type']=='text') && $this->S('dbtype')=='mysql' ? '(255)' : '';
				if ($fieldDef['field_type'] == 'multi') {
					$multis[]= $this->makeMultiQuery($tableName,$fieldDef['field_name'],'ADD');				
					$indexes[]= $this->makeMultiIndex($tableName,$fieldDef['field_name']);				
				} else {
					$sql .= ",\n\t" . $this->makeColumnLine($fieldDef);
					$indexes[] = "CREATE INDEX ".$this->escapeDataTableName($tableName."_".$fieldDef['field_name'])." on " . $this->escapeDataTableName($tableName) . "(".$fname.$ftype.");\n";
				}
			}			
			$sql .= "\n);";
			
			$indexes[] = "\n\tCREATE INDEX ".$this->escapeDataTableName($tableName."_page_id")." on " . $this->escapeDataTableName($tableName) . " (_page_id);\n";
			$indexes[] = "\tCREATE INDEX ".$this->escapeDataTableName($tableName."_page_ns")." on " . $this->escapeDataTableName($tableName) . "(_page_ns);\n";
			$indexes[] = "\tCREATE INDEX ".$this->escapeDataTableName($tableName."_page_title")." on " . $this->escapeDataTableName($tableName) . "(_page_title);\n";
			$indexes[] = "\tCREATE INDEX ".$this->escapeDataTableName($tableName."_page_name")." on " . $this->escapeDataTableName($tableName) . "(_row_name);\n";

			if ($getSqlOnly) return $sql . implode("\n",$multis) . implode("\n",$indexes);
  
      print $sql . implode("\n",$multis) .  implode("\n",$indexes);
      
			$dbr->query($sql);
			foreach($multis as $sql) {
				if ($sql) $dbr->query($sql);
			}
		...
		else
		{
			...
			$fieldDefs=$tableDef['fieldsByNumber'];
			if (count($fieldDefs)==0) { 
			   $this->deleteTableData($tableTitle);
			   return false;
         }
			...
			if (count($alterBits))
			{
				$sql = "ALTER TABLE " . $this->escapeDataTableName($tableName) . "\n";
				$sql.= join(",",$alterBits);
				$sql.=";\n\n";
...
#				print $sql;
		...
		$dbr->insert($this->getInternalTableName('fields'),$fieldDefs);		
		$dbr->commit();
		return true;
	}
...
#				#also add to wiki "what links here (include)"
			...
			$dbr->insert($this->getInternalTableName('links'), $linkTableRows);
		}			
#		$wikiDbr->insert('templatelinks',$templateLinkTableRows); #TODO: replace by wiki's internal mechanism
	...
	function saveMultiData(&$parserOutput,$pageTitle, $pageId)
	{
		...
		if (is_array($outputRows)) {
			...
			$dbr->insert($this->getInternalTableName('multilinks'), $linkTableRows);
		}			
	}
	...
	function deleteTableData(&$title,$pageId=null)  #this really only needs the title, since we can figure everything out from that
	{
		...
		$dbr->delete( $this->getInternalTableName('fields'), array( 'field_table' => $tableName ) );
    $dbr->delete( $this->getInternalTableName('links'), array( 'ln_title' => $tableName ) );

		$sql = "DROP TABLE " . $this->escapeDataTableName($tableName);
		if ($this->tableExists($tableName)) $dbr->query($sql);
...
#		print "fetch for $pageId<br>---<br>";
		while ($res && $row=$dbr->fetchRow($res))
		{
#			print "fetched<br>---<br>";
#			print_r($row);
			$tableName=$this->getDataTableName($row['ln_title']);
...
#		print "done<br>---<br>";
		...
		$dbr->delete( $this->getInternalTableName('links'), array( 'ln_from' => $pageId ) );
	...
	function deleteMultiData(&$title,$pageId )
    {
		...
		$dbr->delete( $this->getInternalTableName('multilinks'), array( 'ml_from' => $pageId ) );		
	}
  	...
  	if (!$dbr) return true; 
   	...
   	if ($newTitle->getNamespace()==NS_XSSDATA) return false; # TODO: delete row data if page moved to Data: namespace
		...
		while ($row=$dbr->fetchRow($res))
		{
			...
			{
				$sql = "UPDATE ". $this->escapeDataTableName($row['ln_title']) . " SET "
...
###################################
	...
	function hook_LinksUpdate(&$linksUpdate) # TODO: Figure out how it works
	{
