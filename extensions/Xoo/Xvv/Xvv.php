...
#  {{#fetch:og|url|field_name,field_name}}
#
############################
...
//echo 'loaded text<br>';
			...
			foreach ($items as $item) {
				...
				$item_array = array(
					'source' => (string)$data->channel->title,
					'title' => (string)$item->title,
					'description' => (string)$item->description,
					'content' => (string)$item->content,
					'link' => (string)($item->guid ? $item->guid : $item->link) ,
					'date' => (string)$item->pubDate,
					'image' => (string)$attrib['url'],
					'media' => (string)$attrib['url'],
					'mediatype' => (string)$attrib['type']
				);				
...
//echo 'loaded text<br><pre>'.print_r($items,1).'</pre>';
			...
			$wgFeedCfg = array(
			//LjudmilaLab
				'facebook_app_id' => '270853126360458', 
				'facebook_secret' => 'a52b2b0afa8d6a918fb6c15fdf1cba1c',
			//Oauth for Mitja Doma
				'fb_oauth_code' => 'AQCPW1X4Bgjd086v2eY47qGeSucMXw7qYySxYhaQV_YRjABK3j9OgcNcSOvUbLMHf_OzqhOFxbt4UacwODkIffQXNo5YWuXOWhypun8f-hC9TfulIVx4LCdIobF3wqSuosm3jhumT1OgcKol-s18656fKCuE7RDKmFydqu7OLcpbumtv4A0JIte9U9EilLaQuIA',
			);
			// get (temp) token for validated user via user's oauth code
			$data = file_get_contents(
				"https://graph.facebook.com/oauth/access_token".
				"?client_id={$wgFeedCfg['facebook_app_id']}".
				"&redirect_uri=http://wiki.ljudmila.org/".
				"&client_secret={$wgFeedCfg['facebook_secret']}".
				"&code={$wgFeedCfg['fb_oauth_code']}"
			);
			$token = false;
			$token = @explode('&',$data);
			...
			if (!$token) return  <<<OUT
	https://www.facebook.com/dialog/oauth?client_id={$wgFeedCfg['facebook_app_id']}&redirect_uri=http://wiki.ljudmila.org&scope=manage_pages,publish_stream,offline_access&state=fangidibula
OUT;
			
			$url = $args->trimExpand(1);
			$fields = $args->trimExpand(2);
			if ($fields) {
				$flds = explode(',',$fields);
				$fields = "&fields=$fields";
			}
			$sep = strpos($url,'?')===false ? '?' : '&';
			$url = "https://graph.facebook.com/{$url}{$sep}access_token=$token$fields";
			$data = file_get_contents($url);
...
//	echo 'data <pre>'.print_r($data,1).'</pre>';
			if ($data->data) {
				foreach ($data->data as $item) {
					$item_array = array();
					foreach ($flds as $fld) {
						$item_array[$fld] = $item->$fld;
					}
					if (!preg_match('#(Wall Photos|Cover Photos)#',$item_array['name'])) {
						$id = $this->arrMake($item_array);
						$arr[] = $id;
					}
				}
			...
			$sql = $args->trimExpand(1);
  			$item_array = array();
	    	if (stripos($sql,'SELECT')===0) {
	    		$res = $dbr->query($sql);
	    		while ( $row = $dbr->fetchRow($res) ) {
//echo '$row <pre>'.print_r($row,1).'</pre>';
	    			$item_array[] = $this->arrMake($row);
	    		}
	    		return array( '0' => $this->arrMake($item_array));
	    	} else {
	    		return array( '0' => $dbr->lastError());
	    	}
...
#			unset ($thisArray[':$:']);
	...
	function fl_this(&$parser, $frame, $a, $parentCount=0)
	{
		...
		case 'rawarg':
			...
			elseif ($args->count > $base+1) {
			  $val = $args->cropExpand($base+2);
			} 
			else 
			{
				return array('found'=>false);
